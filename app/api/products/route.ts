...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ products: await listProducts({ onlyAvailable }) });
}
...
export async function POST(request: NextRequest) {
  ...
  let body: { category?: string; name?: string; description?: string; price?: number; image?: string; available?: boolean };
  try {
